...
namespace TestNote.Service.Service
{
    public class NoteService : BaseService, INoteService
    {
        ...
        {
            return UnitOfWork.GetRepository<Notes>().All.Select(note => new
            {
                note.Id,
                note.Content,
                note.CreateDate,
            }).ToList().Select(note => new NoteModel
            {
        ...
        public List<NoteModel> GetNotesByDateRange(DateTime startDate, DateTime endDate)
        {

            return UnitOfWork.GetRepository<Notes>().AllIncluding(note => note.User)
                .Where(note => note.CreateDate >= startDate && note.CreateDate <= endDate)
                .Select(note => new
                {
                    note.Id,
                    note.Content,
